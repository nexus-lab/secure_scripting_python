...
try:
  for i in range(1,len(sys.argv)):
    if sys.argv[i] == '-g':
      ...
      DELMASTER = "yes"
    elif sys.argv[i] != '-g' or sys.argv[1] != '-d':
      print("Unknown option: " + sys.argv[1])
      sys.exit(1)
except IndexError:
	pass

if DELMASTER == 'yes' and GENMASTER == 'yes':
	print("Only one of –d, -g allowed")
	exit(1)

#Deletes Masterlist if option is given
if DELMASTER == 'yes':
	if not os.path.isfile(MASTER):
		print("Master file does not exist; please generate it")
		exit(1)
	else:
		subprocess.Popen(["rm {}".format(MASTER)],
